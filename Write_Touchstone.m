function touchstone = Write_Touchstone(Matriz_S, Frecuencias, Nombre, Formato,unidad_frecuencia,Z0)
sz = size(Matriz_S);
Formato = lower(Formato);
archivo = append(Nombre,'.s',num2str(sz(1,1)),'p')
...
switch Formato
    ...
    case 'ri'
        ...
        fprintf(fileID,'%s %1s %1s %1s %1s %1s\n','#',Formato_frec,'S',Formato_2,'R',num2str(Z0));
        fprintf(fileID,'\n');
        a = real(Matriz_S);
        b = imag(Matriz_S);
